@extends("base")

@section("body")

    <main>

        <a href="{{ route('sale.download') }}" class="p-4 flex justify-center w-full gap-2 border-b border-b-slate-200 bg-slate-50 text-lg items-center">
                Télécharger l'activité
                <svg class="size-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M1 14.5C1 12.1716 2.22429 10.1291 4.06426 8.9812C4.56469 5.044 7.92686 2 12 2C16.0731 2 19.4353 5.044 19.9357 8.9812C21.7757 10.1291 23 12.1716 23 14.5C23 17.9216 20.3562 20.7257 17 20.9811L7 21C3.64378 20.7257 1 17.9216 1 14.5ZM16.8483 18.9868C19.1817 18.8093 21 16.8561 21 14.5C21 12.927 20.1884 11.4962 18.8771 10.6781L18.0714 10.1754L17.9517 9.23338C17.5735 6.25803 15.0288 4 12 4C8.97116 4 6.42647 6.25803 6.0483 9.23338L5.92856 10.1754L5.12288 10.6781C3.81156 11.4962 3 12.927 3 14.5C3 16.8561 4.81833 18.8093 7.1517 18.9868L7.325 19H16.675L16.8483 18.9868ZM13 12H16L12 17L8 12H11V8H13V12Z"></path></svg>
        </a>

        <section class="divide-y-1 divide-slate-200" id="sales">
            @foreach($sales as $sale)
                <div class="grid gap-4 p-4" style="grid-template-columns: 1fr 100px max-content max-content">
                    <div class="font-bold text-slate-900">{{ $sale->product->name }}</div>
                    <div class="font-bold">{{ $sale->product->price / 100 }} €</div>
                    <div>{{ $sale->author }}</div>
                    <div>{{ $sale->created_at->isoFormat('HH:mm') }}</div>
                </div>
            @endforeach
        </section>

        @if ($sales->hasMorePages())
            <button
                hx-indicator="#spinner"
                hx-trigger="revealed"
                hx-ext="multi-swap"
                hx-disabled-elt="this"
                hx-get="{{ $sales->nextPageUrl() }}"
                hx-swap="multi:#sales:beforeend,#more:outerHTML"
                id="more"
            >
                Page suivante
            </button>
        @else
            <div id="more"></div>
        @endif

    </main>

    <footer class="p-4 shadow-sm bg-white/80 flex justify-between fixed bottom-0 left-0 right-0 backdrop-blur-md">
        <div class="text-xl">
            Total
        </div>
        <div class="font-semibold text-slate-900 text-2xl">
            {{ $total / 100 }} €
        </div>
    </footer>
@endsection
